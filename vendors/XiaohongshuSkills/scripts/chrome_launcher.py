...
def _split_user_data_and_profile(path: str) -> tuple[str, Optional[str]]:
    """
    Interpret account path as either:
    1) Chrome user-data-dir root, or
    2) Specific profile directory under a user-data-dir root (e.g. Default, Profile 1).
    """
    normalized = os.path.abspath(path)
    base = os.path.basename(normalized)
    if base == "Default" or base.startswith("Profile "):
        return os.path.dirname(normalized), base
...
def launch_chrome(
    ...
    user_data_path = get_user_data_dir(account)
    user_data_dir, profile_dir = _split_user_data_and_profile(user_data_path)
