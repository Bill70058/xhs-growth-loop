...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  RUN_LOG="$TMP_DIR/publish_$(date +%F_%H%M%S).log"
  ...
  STATUS="FAILED"
  if rg -q "PUBLISH_STATUS: PUBLISHED" "$RUN_LOG"; then
    STATUS="PUBLISHED"
  elif rg -q "FILL_STATUS: READY_TO_PUBLISH" "$RUN_LOG"; then
...
conn.commit()
conn.close()
print("[publish-record] inserted mode=publish")
PY
